...
class DetailsView extends Component {
  ...
  getContent() {
    ...
    if(audioBlob) {
      const { title, detail, result } = this.state.audioBlob;
      ...
      let otherlist;
      if (result.length > 0) {
        otherlist = result.map((item, index) => {
          return (<li key={`result-${index}`} >
                    <span className="audio-detail">{item[0].name_th} ({item[1]}%)</span>
                  </li>);
        });
      } else {
        otherlist = <span className="audio-detail">{detail}</span>;
      }

      body= (
            <div>
              <div className="details-view-body grey-bg">
                <span className="audio-title">{title}</span>
                {/* <span className="audio-detail">{detail}</span> */}
                {/* <Button
                  className="play"
                  floating={true}
                  icon={buttonIcon}
      ...
      body=(<div className="details-view-body grey-bg">No recording was found</div>);
    }
